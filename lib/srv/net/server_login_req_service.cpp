...
ServerLoginReqService::ServerLoginReqService(ServerConnService& conn_service, PeerType_t peer)
    : conn_service_(conn_service),
      server_table_(conn_service.get_net_base_module().get_server_table()),
      packet_processor_(conn_service.get_net_base_module().get_packet_processor()),
      kSelfPeer(peer)
{
...
void ServerLoginReqService::Login2World(TcpConnection* conn)
{
	...
	packet_processor_.SendPacket2World(msg);
}

void ServerLoginReqService::OnLogout(TcpConnection* conn) {}
...
void ServerLoginReqService::OnMessage_ServerInfoWS(MsgServerInfoWS* msg)
{
    for (int i = 0; i < msg->server_info_size(); ++i) {
        ...
        TcpConnection* conn =
            conn_service_.Connect(si.listen_ip().c_str(), si.listen_port(), nullptr, nullptr);
